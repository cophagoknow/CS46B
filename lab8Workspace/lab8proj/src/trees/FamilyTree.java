...
public class FamilyTree 
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("No colons"); 		//throw a TreeException with a useful message
		...
		else
		{
			...
			if (parentNode == null)
				throw new TreeException("No parent node found in file");	//If the parent node wasn't found, there must have been something wrong in the 
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("No node with name1 found."); //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			 throw new TreeException("No node with name2 found"); //Throw TreeException with a useful message
